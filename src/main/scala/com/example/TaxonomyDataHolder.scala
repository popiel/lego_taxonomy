...
object TaxonomyDataHolder {
  ...
  case class SetTaxonomy(categories: Set[Category], parts: List[LegoPart], replyTo: ActorRef[Response]) extends Command
  ...
  private def idle(categories: Set[Category], parts: List[LegoPart]): Behavior[Command] = Behaviors.receive { (context, message) =>
    message match {
      case SetTaxonomy(newCategories, newParts, replyTo) =>
        ...
        replyTo ! TaxonomyData(newCategories, newParts)
